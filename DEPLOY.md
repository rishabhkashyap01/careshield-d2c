...
3. Under **Environment Variables**, add `DB_POOL_MAX` = `3`.
...
- **Mock payments:** the double-charge protection lives in the database, so it works across function instances. Only the mock gateway's own memory is per-instance, and no real money is involved.
